...
from zbiosynth import data, core, utils
from zbiosynth.core import Registry as R
from zbiosynth.utils import comm, pretty
...
class Engine(core.Configurable):
    ...
    def __init__(self, task, train_set, valid_set, test_set, optimizer, collate_fn=None, scheduler=None, gpus=None, batch_size=1,
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        dataloader = data.DataLoader(self.train_set, self.batch_size, sampler=sampler, num_workers=self.num_worker, collate_fn=self.collate_fn)
        ...
        for epoch in self.meter(num_epoch):
            try:
                if self.scheduler and self.scheduler.name == 'GradualWarmupScheduler':
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                ...
                for key in metric:
                    if f'train_{key}' not in epoch_metrics:
                        epoch_metrics[f'train_{key}'] = []
                    epoch_metrics[f'train_{key}'].append(metric[key].item())
    ...
    def evaluate(self, split, log=True):
        ...
        dataloader = data.DataLoader(test_set, self.batch_size, sampler=sampler, num_workers=self.num_worker, collate_fn=self.collate_fn)
        model = self.model
        ...
        try:
            ...
            metric = model.evaluate(preds, targets)
