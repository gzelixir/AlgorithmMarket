...
@R.register("tasks.SgrnaofftargetPrediction")
class SgrnaofftargetPrediction(tasks.Task, core.Configurable):
    """
    sgrna-dna offtarget prediction task.
    ...
    def __init__(self, model, task=(), criterion="bce", metric=("acc", "bce"), num_mlp_layer=1,
                 ...
                 verbose=0):
        super(SgrnaofftargetPrediction, self).__init__()
        self.model = model
        self.task = task
    ...
    def evaluate(self, pred, target):
        ...
        for _metric in self.metric:
            if _metric == "mae" or _metric == "MAE":
                ...
                score = functional.masked_mean(score, labeled, dim=0)
            elif _metric == "acc" or _metric == "ACC":
            ...
            elif _metric == 'precision' or _metric == 'PRECISION':
                if 'bce' in self.criterion:
                    probs = torch.sigmoid(pred)
                ...
                if 'bce' in self.criterion:
                    ...
                    score = torch.tensor(score)
            elif _metric == "mcc" or _metric == "MCC":
                score = []
                if "bce" in self.criterion:
                    probs = torch.sigmoid(pred)
            ...
            elif _metric == "auroc" or _metric == "AUROC":
                if 'bce' in self.criterion:
                    probs = torch.sigmoid(pred)
