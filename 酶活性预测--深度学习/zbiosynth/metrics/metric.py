...
def matthews_corrcoef(pred, target):
    ...
    return sklearn_matthews_corrcoef(target.cpu().numpy(), pred.cpu().numpy())
...
def pearsonr(pred, target):
    ...
    p = scipy_pearsonr(pred, target)[0]
    return p
...
def spearmanr(pred, target):
    ...
    pred = pred.cpu().numpy()
    target = target.cpu().numpy()
    sp = scipy_spearmanr(pred, target)[0]
    return sp

@R.register("metrics.mcc_score")
...
@R.register("metrics.pre_score")
def pre_score(pred, target, average='binary'):
    pred = np.array(pred)
    target = np.array(target)
    precision = precision_score(target, pred, average=average)
    return precision 

@R.register("metrics.rec_score")
def rec_score(pred, target, average='binary'):
    pred = np.array(pred)
    target = np.array(target)
    recall = recall_score(target, pred, average=average)
    return recall 

@R.register("metrics.f1_score")
def f1_score(pred, target, average='binary'):
    pred = np.array(pred)
    target = np.array(target)
    f1 = sklearn_f1_score(target, pred, average=average)
    return f1 
    
@R.register("metrics.balanced_accuracy")
...
def cal_f1(y_true, y_pred, threshold=0.5):
    ...
    tpfp = np.sum(y_pred_binary, axis=1)
    tpfn = np.sum(y_true, axis=1)

    ave_rec = np.mean(tp/tpfn)
    pre_list = []
    for i in range(len(tp)):
        if tpfp[i] != 0:
            pre_list.append(tp[i]/tpfp[i])
    ave_pre = np.mean(pre_list, axis=0)

    f1 = 2 * ave_pre * ave_rec / (ave_pre + ave_rec)

    return ave_pre, ave_rec, f1

@R.register("metrics.fmax_score")
def fmax_score(inp_y_true, inp_y_pred):
    inp_y_true = inp_y_true.cpu().numpy()
    inp_y_pred = inp_y_pred.sigmoid().cpu().numpy()
    y_true = []
    y_pred = []
    for pred, real in zip(inp_y_pred, inp_y_true):
        if np.sum(real) > 0:
            y_true.append(real)
            y_pred.append(pred)
    ...
    for thres in thresholds:
        ave_pre, ave_rec, f1 = cal_f1(y_true, y_pred, thres)
        if f1 > fmax:
            fmax = f1
            cutoff = thres
            _pre = ave_pre
