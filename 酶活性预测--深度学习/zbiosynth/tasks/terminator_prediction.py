...
@R.register("tasks.TerminatorPrediction")
class TerminatorPrediction(tasks.Task, core.Configurable):
    """
    DNA Transcription Terminator prediction task.
    ...
    def __init__(self, model, task=(), criterion="bce", metric=("acc", "bce"), num_mlp_layer=1,
                 ...
                 verbose=0):
        super(TerminatorPrediction, self).__init__()
        self.model = model
        self.task = task
    ...
    def evaluate(self, pred, target):
        ...
        for _metric in self.metric:
            ...
            elif _metric == 'b_acc' or _metric == 'balanced_acc':
                if 'bce' in self.criterion:
                    probs = torch.sigmoid(pred)
