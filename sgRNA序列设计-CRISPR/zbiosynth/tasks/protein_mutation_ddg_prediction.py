...
@R.register("tasks.ProteinMutationDdgPrediction")
class ProteinMutationDdgPrediction(tasks.Task, core.Configurable):
    """
    Protein mutation ddg prediction task.

    Parameters:
        model (nn.Module): LM representation model
    ...
    def __init__(self, model, task=(), criterion="mse", metric=("pearsonr", "mse"), num_mlp_layer=1,
                 ...
                 verbose=0):
        super(ProteinMutationDdgPrediction, self).__init__()
        self.model = model
        self.task = task
